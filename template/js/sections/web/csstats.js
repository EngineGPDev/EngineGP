$('#install').ajaxForm({
    ...
    success: function (data) {
        $.each(data, function (i, val) {
            ...
            if (i == 's')
                location.href = home + 'servers/id/' + server + '/section/web/subsection/csstats/action/manage';
        });
...
function passwd() {
    ...
    $.getJSON(home + 'servers/id/' + server + '/section/web/subsection/csstats/action/passwd/go', function (arr) {
...
function update() {
    ...
    $.getJSON(home + 'servers/id/' + server + '/section/web/subsection/csstats/action/update/go', function (arr) {
        $.each(arr, function (i, val) {
            if (i == 'e')
                bootbox.dialog('<h3 class="red">Ошибка</h3>' + val,
                    [{
...
function delweb() {
    ...
    $.getJSON(home + 'servers/id/' + server + '/section/web/subsection/csstats/action/delete/go', function (arr) {
        $.each(arr, function (i, val) {
            if (i == 'e')
                bootbox.dialog('<h3 class="red">Ошибка</h3>' + val,
                    [{
...
function connect(key) {
    bootbox.dialog('<div class="input_pad"><input placeholder="Введите лицензионный ключ" id="key"></div>',
        [{
            "label": "Подключить",
            'class': 'btn-success',
            callback: function () {
                ...
                $.getJSON(home + 'servers/id/' + server + '/section/web/subsection/csstats/action/connect/server/' + ser + '/key/' + $('#key').val() + '/go', function (arr) {
                    $.each(arr, function (i, val) {
                        if (i == 'e')
                            bootbox.dialog('<h3 class="red">Ошибка</h3>' + val,
                                [{
