...
if ($go) {
    ...
    $aData['text'] = isset($_POST['text']) ? trim((string) $_POST['text']) : htmlspecialchars_decode((string) $notice['text']);
    $aData['color'] = isset($_POST['color']) ? trim((string) $_POST['color']) : $notice['color'];
    $aData['type'] = isset($_POST['type']) ? trim((string) $_POST['type']) : $notice['type'];
    ...
    if ($aData['type'] == 'unit') {
        ...
        sys::outjs(['e' => 'Выберете получателя уведомления']);
...
$html->set('text', htmlspecialchars_decode((string) $notice['text']));
