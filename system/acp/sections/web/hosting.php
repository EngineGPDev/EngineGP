...
if ($id) {
    ...
    AdminSystem::page_gen($aPage['ceil'], $page, $aPage['page'], 'acp/web');

    $webs = $sql->query('SELECT `id`, `type`, `server`, `user`, `unit`, `domain`, `passwd`, `login`, `date` FROM `web` WHERE `type`="hosting" ORDER BY `id` ASC LIMIT ' . $aPage['num'] . ', 20');
    while ($web = $sql->get($webs)) {
        ...
        $list .= '<td class="text-center"><a href="//' . $web['domain'] . '" target="_blank">' . $web['domain'] . '</a></td>';
