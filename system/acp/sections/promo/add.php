...
if ($go) {
    ...
    $aData['cod'] = isset($_POST['cod']) ? trim((string) $_POST['cod']) : '';
    $aData['value'] = isset($_POST['value']) ? trim((string) $_POST['value']) : '';
    ...
    $aData['data'] = isset($_POST['data']) ? trim((string) $_POST['data']) : '';
...
while ($unit = $sql->get($units)) {
    ...
    while ($tarif = $sql->get())
        $tarifs .= '<label> ' . $unit['name'] . ' / #' . $tarif['id'] . ' ' . $tarif['name'] . ' (' . strtoupper((string) $tarif['game']) . ') <input type="checkbox" name="tarifs[' . $tarif['id'] . ']"></label>';
}
