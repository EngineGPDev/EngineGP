...
class service
{
    public static function buy($aData = [])
    {
        ...
        // Проверка локации
        $sql->query('SELECT `address`, `test` FROM `units` WHERE `id`="' . $aData['unit'] . '" AND `cs2`="1" AND `show`="1" LIMIT 1');
        if (!$sql->num()) {
        ...
        if (!in_array($aData['tickrate'], explode(':', $tarif['tickrate']))) {
            System::outjs(['e' => 'Переданные данные tickrate неверны.']);
        }
        ...
        if ($aData['test']) {
            ...
            if ($sql->num()) {
                ...
                if (!$cfg['tests']['game'] || $test_info['game'] == 'cs2') {
    ...
    public static function install($aSDATA = [])
    {
        ...
        $sshClient->execude('mkdir ' . $install . ';' // Создание директории
            ...
            . 'chmod 500 ' . Parameters::$aFileGame['cs2'] . '"');
        ...
        if ($aSDATA['test']) {
            $sql->query('INSERT INTO `tests` set `server`="' . $id . '", `unit`="' . $aSDATA['unit'] . '", `game`="cs2", `user`="' . $user['id'] . '", `time`="' . $start_point . '"');
