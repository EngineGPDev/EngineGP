...
class action extends actions
{
    public static function start($id, $type = 'start')
    {
        ...
        // Временный файл
        $temp = System::temp(action::config($ip, $port, $server['slots_start'], $sshClient->execute('cat ' . $tarif['install'] . '/' . $server['uid'] . '/mods/deathmatch/mtaserver.conf', false)));

        // Обновление файла server.cfg
        $sftpClient->putFile($temp, $tarif['install'] . $server['uid'] . '/mods/deathmatch/mtaserver.conf');
        $sshClient->execute('chmod 0644' . ' ' . $tarif['install'] . $server['uid'] . '/mods/deathmatch/mtaserver.conf');
