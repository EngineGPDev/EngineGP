...
class action extends actions
{
    ...
    {
        $aRepl = ['<serverip>', '</serverip>', '<maxplayers>', '</maxplayers>', '<httpserver>', '</httpserver>', '<serverport>', '</serverport>', '<httpport>', '</httpport>', '<logfile>', '</logfile>', '<ase>', '</ase>'];

        $config = str_ireplace($aRepl, $aRepl, (string) $config);
        $config = str_ireplace(['<config>', '</config>', "\r", "\t"], '', $config);
        $config = preg_replace("#\<\!--(.+?)--\>#is", '', $config);

        $aLine = explode("\n", $config);

        $search = ["#\<serverip\>(.+?)\<\/serverip\>#is", "#\<maxplayers\>(.+?)\<\/maxplayers\>#is", "#\<httpserver\>(.+?)\<\/httpserver\>#is", "#\<serverport\>(.+?)\<\/serverport\>#is", "#\<httpport\>(.+?)\<\/httpport\>#is", "#\<logfile\>(.+?)\<\/logfile\>#is", "#\<ase\>(.+?)\<\/ase\>#is"];
