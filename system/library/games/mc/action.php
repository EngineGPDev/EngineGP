...
class action extends actions
{
    public static function start($id, $type = 'start')
    {
        ...
        // Временный файл
        $temp = System::temp(action::config($ip, $port, $server['slots_start'], $sshClient->execute('cat ' . $tarif['install'] . '/' . $server['uid'] . '/server.properties', false)));

        // Обновление файла server.cfg
        $sftpClient->putFile($temp, $tarif['install'] . $server['uid'] . '/server.properties');
        $sshClient->execute('chmod 0644' . ' ' . $tarif['install'] . $server['uid'] . '/server.properties');
