...
class service
{
    public static function buy($aData = [])
    {
        ...
        // Проверка RAM
        if (!in_array($aData['ram'], explode(':', (string) $tarif['ram'])))
            sys::outjs(['e' => 'Переданные данные ram неверны.']);
        ...
        $price = $aPrice[array_search($aData['ram'], explode(':', (string) $tarif['ram']))];
        ...
        $promo = games::define_promo(
            $aData['promo'],
            ['tarif' => $aData['tarif'], 'ram' => $aData['ram'], 'slots' => $aData['slots'], 'time' => $aData['time'], 'user' => $user['id']],
            $tarif['discount'],
            $sum
        );
