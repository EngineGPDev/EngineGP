...
class service
{
    public static function buy($aData = [])
    {
        ...
        // Проверка сборки
        if (!array_key_exists($aData['pack'], sys::b64djs($tarif['packs'])))
            sys::outjs(['e' => 'Сборка не найдена.']);

        // Проверка FPS
        if (!in_array($aData['fps'], explode(':', (string) $tarif['fps'])))
            sys::outjs(['e' => 'Переданные данные fps неверны.']);

        // Проверка TickRate
        if (!in_array($aData['tickrate'], explode(':', (string) $tarif['tickrate'])))
            sys::outjs(['e' => 'Переданные данные tickrate неверны.']);
        ...
        if ($aData['test']) {
            ...
            // Проверка периода
            if (!$cfg['settlement_period'] and !in_array($aData['time'], explode(':', (string) $tarif['time'])))
        ...
        $promo = games::define_promo(
            $aData['promo'],
            ['tarif' => $aData['tarif'], 'tickrate' => $aData['tickrate'], 'fps' => $aData['fps'], 'slots' => $aData['slots'], 'time' => $aData['time'], 'user' => $user['id']],
            $tarif['discount'],
            $sum
        );
        ...
        $aSDATA = [
            ...
            'ram' => $ram,
            // значение ram
            'param_fix' => isset($tarif['param_fix']),
            ...
            // значение fps
            'pack' => $aData['pack'],
            // Выбранная сборка для установки
            'time' => $aData['time'],
            // Время аренды
            'days' => $days,
            // Число дней
            ...
            // Кол-во слот
            'map' => $tarif['map'],
            // Фиксированное значение слот
            'autostop' => $tarif['autostop'],
            // Выключение при 0 онлайне
            'ftp' => $tarif['ftp'],
            // Использование ftp
            'plugins' => $tarif['plugins'],
            // Использование плагинов
            'console' => $tarif['console'],
            // Использование консоли
            'stats' => $tarif['stats'],
            // Использование графиков (ведение статистики)
            'copy' => $tarif['copy'],
            // Использование резервных копий
            'web' => $tarif['web'],
            // Использование доп услуг
            'plugins_install' => $tarif['plugins_install'],
    ...
    public static function install($aSDATA = [])
    {
        ...
        if ($aSDATA['plugins']) {
            ...
            if (isset($aPlugins[$aSDATA['pack']])) {
                $plugins = explode(',', (string) $aPlugins[$aSDATA['pack']]);
