...
class pDraw
{
    ...
    function drawPolygon($Points, $Format = "")
    {
        ...
        if (!$NoFill) {
            if ($this->Shadow && $this->ShadowX != 0 && $this->ShadowY != 0) {
                $this->Shadow = FALSE;
                for ($i = 0; $i <= (is_countable($Points) ? count($Points) : 0) - 1; $i = $i + 2) {
    ...
    function drawSpline($Coordinates, $Format = "")
    {
        ...
        $Mode = NULL;
        $Result = [];
        for ($i = 1; $i <= (is_countable($Coordinates) ? count($Coordinates) : 0) - 1; $i++) {
    ...
    function drawText($X, $Y, $Text, $Format = "")
    {
        ...
        if ($this->Shadow && $this->ShadowX != 0 && $this->ShadowY != 0) {
            ...
            imagettftext($this->Picture, $FontSize, $Angle, $X + $this->ShadowX, $Y + $this->ShadowY, $C_ShadowColor, $FontName, (string) $Text);
        }

        $C_TextColor = $this->AllocateColor($this->Picture, $R, $G, $B, $Alpha);
        imagettftext($this->Picture, $FontSize, $Angle, $X, $Y, $C_TextColor, $FontName, (string) $Text);
    ...
    function drawArrowLabel($X1, $Y1, $Text, $Format = "")
    {
        ...
        $Size = imagettfbbox($FontSize, 0, $FontName, (string) $Text);
    ...
    function computeScale($XMin, $XMax, $MaxDivs, $Factors, $AxisID = 0)
    {
        ...
        foreach ($Results as $Key => $Result) {
            $Decimals = preg_split("/\./", (string) $Result["RowHeight"]);
    ...
    */
    ...
    {
        $AxisID = $Option["AxisID"] ?? 0;
        $SerieName = $Option["SerieName"] ?? NULL;
    ...
    function scaleFormat($Value, $Mode = NULL, $Format = NULL, $Unit = NULL)
    {
        ...
        if ($Mode == AXIS_FORMAT_METRIC) {
            if (abs($Value) > 1_000_000_000)
                return (round($Value / 1_000_000_000, $Format) . "g" . $Unit);
            if (abs($Value) > 1_000_000)
                return (round($Value / 1_000_000, $Format) . "m" . $Unit);
    ...
    {
        $MaxLabelTxt = $Format["MaxLabelTxt"] ?? "max=";
        $MinLabelTxt = $Format["MinLabelTxt"] ?? "min=";
        $Decimals = $Format["Decimals"] ?? 1;
        $ExcludedSeries = $Format["ExcludedSeries"] ?? "";
        $DisplayOffset = $Format["DisplayOffset"] ?? 4;
        $DisplayColor = $Format["DisplayColor"] ?? DISPLAY_MANUAL;
        $MaxDisplayR = $Format["MaxDisplayR"] ?? 0;
        $MaxDisplayG = $Format["MaxDisplayG"] ?? 0;
        $MaxDisplayB = $Format["MaxDisplayB"] ?? 0;
        $MinDisplayR = $Format["MinDisplayR"] ?? 255;
        $MinDisplayG = $Format["MinDisplayG"] ?? 255;
        $MinDisplayB = $Format["MinDisplayB"] ?? 255;
        $MinLabelPos = $Format["MinLabelPos"] ?? BOUND_LABEL_POS_AUTO;
        $MaxLabelPos = $Format["MaxLabelPos"] ?? BOUND_LABEL_POS_AUTO;
        $DrawBox = $Format["DrawBox"] ?? TRUE;
        $DrawBoxBorder = $Format["DrawBoxBorder"] ?? FALSE;
        $BorderOffset = $Format["BorderOffset"] ?? 5;
        $BoxRounded = $Format["BoxRounded"] ?? TRUE;
    ...
    {
        $PlotSize = $Format["PlotSize"] ?? NULL;
        $PlotBorder = $Format["PlotBorder"] ?? FALSE;
        $BorderR = $Format["BorderR"] ?? 50;
        $BorderG = $Format["BorderG"] ?? 50;
        $BorderB = $Format["BorderB"] ?? 50;
        $BorderAlpha = $Format["BorderAlpha"] ?? 30;
        $BorderSize = $Format["BorderSize"] ?? 2;
        $Surrounding = $Format["Surrounding"] ?? NULL;
        $DisplayValues = $Format["DisplayValues"] ?? FALSE;
        $DisplayOffset = $Format["DisplayOffset"] ?? 4;
        $DisplayColor = $Format["DisplayColor"] ?? DISPLAY_MANUAL;
        $DisplayR = $Format["DisplayR"] ?? 0;
        $DisplayG = $Format["DisplayG"] ?? 0;
        $DisplayB = $Format["DisplayB"] ?? 0;
    ...
    {
        $BreakVoid = $Format["BreakVoid"] ?? TRUE;
        $VoidTicks = $Format["VoidTicks"] ?? 4;
        $BreakR = $Format["BreakR"] ?? NULL; // 234
        $BreakG = $Format["BreakG"] ?? NULL; // 55
        $BreakB = $Format["BreakB"] ?? NULL; // 26
        $DisplayValues = $Format["DisplayValues"] ?? FALSE;
        $DisplayOffset = $Format["DisplayOffset"] ?? 2;
        $DisplayColor = $Format["DisplayColor"] ?? DISPLAY_MANUAL;
        $DisplayR = $Format["DisplayR"] ?? 0;
        $DisplayG = $Format["DisplayG"] ?? 0;
        $DisplayB = $Format["DisplayB"] ?? 0;
        $RecordImageMap = $Format["RecordImageMap"] ?? FALSE;
        $ImageMapPlotSize = $Format["ImageMapPlotSize"] ?? 5;
    ...
    function drawFilledSplineChart($Format = NULL)
    {
        ...
        $DisplayG = $Format["DisplayG"] ?? 0;
        $DisplayB = $Format["DisplayB"] ?? 0;
        $AroundZero = $Format["AroundZero"] ?? TRUE;
        $Threshold = $Format["Threshold"] ?? NULL;
                            ...
                            if ($Area != "") {
                                foreach ($Area as $key => $Points) {
                                    ...
                                    foreach ($Points as $subKey => $Point) {
                                        if ($subKey == (is_countable($Points) ? count($Points) : 0) - 1) {
                                            $Corners[] = $Point["X"] - 1;
                                        } else {
                                            $Corners[] = $Point["X"];
                                        }
    ...
    function drawLineChart($Format = NULL)
    {
        $BreakVoid = $Format["BreakVoid"] ?? TRUE;
        $VoidTicks = $Format["VoidTicks"] ?? 4;
        $BreakR = $Format["BreakR"] ?? NULL;
        $BreakG = $Format["BreakG"] ?? NULL;
        $BreakB = $Format["BreakB"] ?? NULL;
        $DisplayValues = $Format["DisplayValues"] ?? FALSE;
        $DisplayOffset = $Format["DisplayOffset"] ?? 2;
        $DisplayColor = $Format["DisplayColor"] ?? DISPLAY_MANUAL;
        $DisplayR = $Format["DisplayR"] ?? 0;
        ...
        $RecordImageMap = $Format["RecordImageMap"] ?? FALSE;
        $ImageMapPlotSize = $Format["ImageMapPlotSize"] ?? 5;
        $ForceColor = $Format["ForceColor"] ?? FALSE;
        $ForceR = $Format["ForceR"] ?? 0;
        $ForceG = $Format["ForceG"] ?? 0;
        $ForceB = $Format["ForceB"] ?? 0;
        $ForceAlpha = $Format["ForceAlpha"] ?? 100;
    ...
    {
        $AxisID = $Format["AxisID"] ?? 0;
        $LineR = $Format["LineR"] ?? 150;
        $LineG = $Format["LineG"] ?? 150;
        $LineB = $Format["LineB"] ?? 150;
        $LineAlpha = $Format["LineAlpha"] ?? 50;
        $LineTicks = $Format["LineTicks"] ?? 1;
        $AreaR = $Format["AreaR"] ?? 150;
        $AreaG = $Format["AreaG"] ?? 150;
        $AreaB = $Format["AreaB"] ?? 150;
        $AreaAlpha = $Format["AreaAlpha"] ?? 5;
    ...
    {
        $BreakVoid = $Format["BreakVoid"] ?? FALSE;
        $ReCenter = $Format["ReCenter"] ?? TRUE;
        $VoidTicks = $Format["VoidTicks"] ?? 4;
        $BreakR = $Format["BreakR"] ?? NULL;
        $BreakG = $Format["BreakG"] ?? NULL;
        $BreakB = $Format["BreakB"] ?? NULL;
    ...
    function drawFilledStepChart($Format = NULL)
    {
        ...
        $DisplayOffset = $Format["DisplayOffset"] ?? 2;
        $DisplayColor = $Format["DisplayColor"] ?? DISPLAY_MANUAL;
        $ForceTransparency = $Format["ForceTransparency"] ?? NULL;
        $DisplayR = $Format["DisplayR"] ?? 0;
        $DisplayG = $Format["DisplayG"] ?? 0;
        $DisplayB = $Format["DisplayB"] ?? 0;
        $AroundZero = $Format["AroundZero"] ?? TRUE;
    ...
    function drawAreaChart($Format = NULL)
    {
        ...
        $DisplayG = $Format["DisplayG"] ?? 0;
        $DisplayB = $Format["DisplayB"] ?? 0;
        $ForceTransparency = $Format["ForceTransparency"] ?? 25;
        $AroundZero = $Format["AroundZero"] ?? TRUE;
        $Threshold = $Format["Threshold"] ?? NULL;
    ...
    {
        $Floating0Serie = $Format["Floating0Serie"] ?? NULL;
        $Floating0Value = $Format["Floating0Value"] ?? NULL;
        $Draw0Line = $Format["Draw0Line"] ?? FALSE;
        $DisplayValues = $Format["DisplayValues"] ?? FALSE;
        $DisplayOrientation = $Format["DisplayOrientation"] ?? ORIENTATION_HORIZONTAL;
        $DisplayOffset = $Format["DisplayOffset"] ?? 2;
        $DisplayColor = $Format["DisplayColor"] ?? DISPLAY_MANUAL;
        $DisplayFont = $Format["DisplaySize"] ?? $this->FontName;
        $DisplaySize = $Format["DisplaySize"] ?? $this->FontSize;
        $DisplayPos = $Format["DisplayPos"] ?? LABEL_POS_OUTSIDE;
        $DisplayShadow = $Format["DisplayShadow"] ?? TRUE;
        $DisplayR = $Format["DisplayR"] ?? 0;
        $DisplayG = $Format["DisplayG"] ?? 0;
        $DisplayB = $Format["DisplayB"] ?? 0;
        $AroundZero = $Format["AroundZero"] ?? TRUE;
        $Interleave = $Format["Interleave"] ?? .5;
        $Rounded = $Format["Rounded"] ?? FALSE;
        $RoundRadius = $Format["RoundRadius"] ?? 4;
        $Surrounding = $Format["Surrounding"] ?? NULL;
        $BorderR = $Format["BorderR"] ?? -1;
        $BorderG = $Format["BorderG"] ?? -1;
        $BorderB = $Format["BorderB"] ?? -1;
        $Gradient = $Format["Gradient"] ?? FALSE;
        $GradientMode = $Format["GradientMode"] ?? GRADIENT_SIMPLE;
        $GradientAlpha = $Format["GradientAlpha"] ?? 20;
        $GradientStartR = $Format["GradientStartR"] ?? 255;
        $GradientStartG = $Format["GradientStartG"] ?? 255;
        $GradientStartB = $Format["GradientStartB"] ?? 255;
        $GradientEndR = $Format["GradientEndR"] ?? 0;
        $GradientEndG = $Format["GradientEndG"] ?? 0;
        $GradientEndB = $Format["GradientEndB"] ?? 0;
        $TxtMargin = $Format["TxtMargin"] ?? 6;
        $OverrideColors = $Format["OverrideColors"] ?? NULL;
        $OverrideSurrounding = $Format["OverrideSurrounding"] ?? 30;
        $InnerSurrounding = $Format["InnerSurrounding"] ?? NULL;
        $InnerBorderR = $Format["InnerBorderR"] ?? -1;
        $InnerBorderG = $Format["InnerBorderG"] ?? -1;
        $InnerBorderB = $Format["InnerBorderB"] ?? -1;
        $RecordImageMap = $Format["RecordImageMap"] ?? FALSE;
    ...
    {
        $DisplayValues = $Format["DisplayValues"] ?? FALSE;
        $DisplayOrientation = $Format["DisplayOrientation"] ?? ORIENTATION_AUTO;
        $DisplayRound = $Format["DisplayRound"] ?? 0;
        $DisplayColor = $Format["DisplayColor"] ?? DISPLAY_MANUAL;
        $DisplayFont = $Format["DisplayFont"] ?? $this->FontName;
        $DisplaySize = $Format["DisplaySize"] ?? $this->FontSize;
        $DisplayR = $Format["DisplayR"] ?? 0;
        $DisplayG = $Format["DisplayG"] ?? 0;
        $DisplayB = $Format["DisplayB"] ?? 0;
        ...
        $InnerBorderB = $Format["InnerBorderB"] ?? -1;
        $RecordImageMap = $Format["RecordImageMap"] ?? FALSE;
        $FontFactor = $Format["FontFactor"] ?? 8;
    ...
    {
        $DrawLine = $Format["DrawLine"] ?? FALSE;
        $LineSurrounding = $Format["LineSurrounding"] ?? NULL;
        $LineR = $Format["LineR"] ?? VOID;
        $LineG = $Format["LineG"] ?? VOID;
        $LineB = $Format["LineB"] ?? VOID;
        $LineAlpha = $Format["LineAlpha"] ?? 100;
        $DrawPlot = $Format["DrawPlot"] ?? FALSE;
        $PlotRadius = $Format["PlotRadius"] ?? 2;
        $PlotBorder = $Format["PlotBorder"] ?? 1;
        $PlotBorderSurrounding = $Format["PlotBorderSurrounding"] ?? NULL;
        $PlotBorderR = $Format["PlotBorderR"] ?? 0;
        $PlotBorderG = $Format["PlotBorderG"] ?? 0;
        $PlotBorderB = $Format["PlotBorderB"] ?? 0;
        $PlotBorderAlpha = $Format["PlotBorderAlpha"] ?? 50;
        $ForceTransparency = $Format["ForceTransparency"] ?? NULL;
    ...
    {
        return (["R" => random_int(0, 255), "G" => random_int(0, 255), "B" => random_int(0, 255), "Alpha" => $Alpha]);
    }
    ...
    function validatePalette($Colors, $Surrounding = NULL)
    {
        ...
        foreach ($Colors as $Key => $Values) {
            if (isset($Values["R"])) {
                ...
                $Result[$Key]["R"] = random_int(0, 255);
            }
            if (isset($Values["G"])) {
                $Result[$Key]["G"] = $Values["G"];
            } else {
                $Result[$Key]["G"] = random_int(0, 255);
            }
            if (isset($Values["B"])) {
                $Result[$Key]["B"] = $Values["B"];
            } else {
                $Result[$Key]["B"] = random_int(0, 255);
            }
    ...
    {
        $Offset = $Format["Offset"] ?? 10;
        $SerieSpacing = $Format["SerieSpacing"] ?? 3;
        $DerivativeHeight = $Format["DerivativeHeight"] ?? 4;
        $ShadedSlopeBox = $Format["ShadedSlopeBox"] ?? FALSE;
        $DrawBackground = $Format["DrawBackground"] ?? TRUE;
        $BackgroundR = $Format["BackgroundR"] ?? 255;
        $BackgroundG = $Format["BackgroundG"] ?? 255;
        $BackgroundB = $Format["BackgroundB"] ?? 255;
        $BackgroundAlpha = $Format["BackgroundAlpha"] ?? 20;
        $DrawBorder = $Format["DrawBorder"] ?? TRUE;
        $BorderR = $Format["BorderR"] ?? 0;
        $BorderG = $Format["BorderG"] ?? 0;
        $BorderB = $Format["BorderB"] ?? 0;
        $BorderAlpha = $Format["BorderAlpha"] ?? 100;
        $Caption = $Format["Caption"] ?? TRUE;
        $CaptionHeight = $Format["CaptionHeight"] ?? 10;
        $CaptionWidth = $Format["CaptionWidth"] ?? 20;
        $CaptionMargin = $Format["CaptionMargin"] ?? 4;
        $CaptionLine = $Format["CaptionLine"] ?? FALSE;
        $CaptionBox = $Format["CaptionBox"] ?? FALSE;
        $CaptionBorderR = $Format["CaptionBorderR"] ?? 0;
        $CaptionBorderG = $Format["CaptionBorderG"] ?? 0;
        $CaptionBorderB = $Format["CaptionBorderB"] ?? 0;
        $CaptionFillR = $Format["CaptionFillR"] ?? 255;
        $CaptionFillG = $Format["CaptionFillG"] ?? 255;
        $CaptionFillB = $Format["CaptionFillB"] ?? 255;
        $CaptionFillAlpha = $Format["CaptionFillAlpha"] ?? 80;
        $PositiveSlopeStartR = $Format["PositiveSlopeStartR"] ?? 184;
        $PositiveSlopeStartG = $Format["PositiveSlopeStartG"] ?? 234;
        $PositiveSlopeStartB = $Format["PositiveSlopeStartB"] ?? 88;
        $PositiveSlopeEndR = $Format["PositiveSlopeStartR"] ?? 239;
        $PositiveSlopeEndG = $Format["PositiveSlopeStartG"] ?? 31;
        $PositiveSlopeEndB = $Format["PositiveSlopeStartB"] ?? 36;
        $NegativeSlopeStartR = $Format["NegativeSlopeStartR"] ?? 184;
        $NegativeSlopeStartG = $Format["NegativeSlopeStartG"] ?? 234;
        $NegativeSlopeStartB = $Format["NegativeSlopeStartB"] ?? 88;
        $NegativeSlopeEndR = $Format["NegativeSlopeStartR"] ?? 67;
        $NegativeSlopeEndG = $Format["NegativeSlopeStartG"] ?? 124;
        $NegativeSlopeEndB = $Format["NegativeSlopeStartB"] ?? 227;
    ...
    {
        $OverrideTicks = $Format["Ticks"] ?? NULL;
        $OverrideR = $Format["R"] ?? VOID;
        $OverrideG = $Format["G"] ?? VOID;
        $OverrideB = $Format["B"] ?? VOID;
        $OverrideAlpha = $Format["Alpha"] ?? VOID;
        ...
        foreach ($Data["Series"] as $SerieName => $Serie) {
            if ($Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"]) {
                ...
                if ($Data["Orientation"] == SCALE_POS_LEFTRIGHT) {
                    ...
                    $n = is_countable($this->DataSet->stripVOID($PosArray)) ? count($this->DataSet->stripVOID($PosArray)) : 0; //$n = count($PosArray);
    ...
    function drawLabelBox($X, $Y, $Title, $Captions, $Format = "")
    {
        ...
        $TitleAlpha = $Format["TitleAlpha"] ?? 100;
        ...
        $TitleBackgroundAlpha = $Format["TitleBackgroundAlpha"] ?? 100;
        $GradientStartR = $Format["GradientStartR"] ?? 255;
        $GradientStartG = $Format["GradientStartG"] ?? 255;
        $GradientStartB = $Format["GradientStartB"] ?? 255;
    ...
    function drawPolygonChart($Points, $Format = "")
    {
        ...
        $AllIntegers = TRUE;
        for ($i = 0; $i <= (is_countable($Points) ? count($Points) : 0) - 2; $i = $i + 2) {
        ...
        $Segments = [];
        for ($i = 2; $i <= (is_countable($Points) ? count($Points) : 0) - 2; $i = $i + 2) {
