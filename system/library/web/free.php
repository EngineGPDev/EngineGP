...
class web
{
    public static function install($mcache, $aData = [])
    {
        ...
        $conf = ['address' => $aData['server']['address'], 'install' => $install, 'login' => $login, 'passwd' => $passwd, 'host' => $host, 'url' => $cfg['http'], 'domain' => $aData['subdomain'] . '.' . $aData['domain']];
        ...
        // Создание поддомена
        $result = json_decode(file_get_contents(sys::updtext($aWebUnit['isp']['domain']['create'], ['subdomain' => $aData['subdomain'], 'ip' => $ip, 'domain' => $aData['domain']])), true, 512, JSON_THROW_ON_ERROR);
        ...
        if (isset($aWebUnit['isp']['crontab'][$aData['type']]['install'])) {
            $result = json_decode(file_get_contents(sys::updtext($aWebUnit['isp']['crontab'][$aData['type']]['install'], ['subdomain' => $aData['subdomain'], 'domain' => $aData['domain']])), true, 512, JSON_THROW_ON_ERROR);
            if (!isset($result['result']) || strtolower((string) $result['result']) != 'ok') {
                $sql->query('DELETE FROM `web` WHERE `id`="' . $wid . '" LIMIT 1');
            ...
            if (isset($aWebSQL[$aData['type']]['install']))
                foreach ($aWebSQL[$aData['type']]['install'] as $query)
                    $sql_q .= "mysql --login-path=local " . $login . " -e \"" . sys::updtext($query,
                            ['url' => $cfg['http'], 'passwd' => $aData['passwd'], 'mail' => $u['mail'], 'folder' => $install]
                        ) . "\";";
        }
        ...
        $sql->query('UPDATE `web` set `uid`="' . $uid . '", `desing`="' . $aData['desing'] . '", '
            ...
            . '`passwd`="' . $aData['passwd'] . '", `config`="' . base64_encode((string) $aData['config_sql']) . '", '
    ...
    public static function update($mcache, $aData = [])
    {
    ...
    public static function delete($mcache, $aData = [])
    {
        global $sql;
        ...
        $aDomain = explode('.', (string) $web['domain']);
        $zone = array_pop($aDomain);

        // Удаление поддомена
        if ($aData['type'] != 'mysql') {
            $result = json_decode(file_get_contents(sys::updtext($aWebUnit['isp']['domain']['delete'], ['subdomain' => $web['domain'], 'domain' => end($aDomain) . '.' . $zone, 'ip' => $ip])), true, 512, JSON_THROW_ON_ERROR);
        ...
        if (isset($aWebUnit['isp']['crontab'][$aData['type']]['delete']) && isset($aData['cron'])) {
            $result = json_decode(file_get_contents(sys::updtext($aWebUnit['isp']['crontab'][$aData['type']]['delete'], ['data' => $aData['cron']])), true, 512, JSON_THROW_ON_ERROR);
            if (!isset($result['result']) || strtolower((string) $result['result']) != 'ok')
    ...
    public static function connect($mcache, $aData = [])
    {
        global $cfg, $sql, $start_point;
        ...
        // Взять rcon_password
        $get = explode(' ', str_replace('"', '', trim((string) $ssh->get('cat ' . $dir . $aData['cfg'] . ' | grep rcon_password'))));
        ...
        $temp = sys::temp(sys::updtext(base64_decode((string) $web['config']), $aData['orcfg']));
        ...
        if (isset($aWebSQL[$aData['type']]['connect']))
            foreach ($aWebSQL[$aData['type']]['connect'] as $query)
                $sql_q .= "mysql --login-path=local " . $web['login'] . " -e \"" . sys::updtext($query,
                        array_merge(['id' => $aData['server']['id'], 'rcon' => $rcon, 'address' => $server['address'], 'ip' => $ip, 'port' => $port, 'name' => $server['name'], 'time' => $start_point], $aData['orsql'])) . "\";";
    ...
    public static function passwd($mcache, $aData = [])
    {
        global $sql;
    ...
    public static function stack($aData, $select = '`id`, `unit`, `login`')
    {
        ...
        require(DATA . 'web.php');
        return match ($aWebInstall[$aData['server']['game']][$aData['type']]) {
            'server' => $sql->query('SELECT ' . $select . ' FROM `web` WHERE `type`="' . $aData['type'] . '" AND `server`="' . $aData['server']['id'] . '" LIMIT 1'),
            'user' => $sql->query('SELECT ' . $select . ' FROM `web` WHERE `type`="' . $aData['type'] . '" AND `user`="' . $aData['server']['user'] . '" LIMIT 1'),
            'unit' => $sql->query('SELECT ' . $select . ' FROM `web` WHERE `type`="' . $aData['type'] . '" AND `user`="' . $aData['server']['user'] . '" AND `unit`="' . $aData['server']['unit'] . '" LIMIT 1'),
            default => NULL,
        };
    }
