...
class api
{
    public function data($id)
    {
        ...
        return ['id' => $id, 'address' => $server['address'], 'unit' => $unit['name'], 'tarif' => games::info_tarif($server['game'], $tarif['name'], ['fps' => $server['fps'], 'tickrate' => $server['tickrate'], 'ram' => $server['ram']]), 'game' => $server['game'], 'name' => $server['name'], 'slots' => $server['slots_start'], 'online' => $server['online'], 'players' => $server['players'], 'status' => sys::status($server['status'], $server['game'], $server['map']), 'img' => sys::status($server['status'], $server['game'], $server['map'], 'img'), 'time_end' => $time_end, 'time' => sys::today($server['time']), 'date' => sys::today($server['date']), 'pack' => $packs[$server['pack']]];
    }
    ...
    public function load($id)
    {
        ...
        return ['id' => $id, 'cpu' => $server['cpu_use'], 'ram' => $server['ram_use'], 'hdd' => $server['hdd_use'], 'onl' => $online];
    }
    ...
    public function console($id, $cmd)
    {
        ...
        $aGames = ['cs', 'css', 'cssold', 'csgo', 'mc', 'mta'];
        ...
        $_POST['command'] = isset($cmd[0]) ? urldecode((string) $cmd) : '';
