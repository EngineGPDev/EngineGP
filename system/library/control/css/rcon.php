...
class rcon
{
    public static function cmd($server, $cmd = 'status')
    {
        ...
        require(LIB . 'games/query/SourceRcon.php');
    ...
    public static function rcon_passwd($server)
    {
        ...
        if (!isset($rcon[0]))
            sys::outjs(['r' => 'Необходимо установить rcon пароль (rcon_password).', 'url' => $cfg['http'] . 'control/id/' . $server['unit'] . '/server/' . $server['unit'] . '/section/settings/subsection/server'], $nmch);
