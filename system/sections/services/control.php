...
if ($go) {
    ...
    $aData = ['address' => isset($_POST['address']) ? trim((string) $_POST['address']) : 0, 'passwd' => $_POST['passwd'] ?? 0, 'time' => isset($_POST['time']) ? sys::int($_POST['time']) : 30, 'limit' => isset($_POST['limit']) ? sys::int($_POST['limit']) : key(array_shift($cfg['control_limit']))];
