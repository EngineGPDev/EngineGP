...
if ($go) {
    ...
    // Входные данные
    $aData = ['unit' => isset($_POST['unit']) ? sys::int($_POST['unit']) : 0, 'tarif' => isset($_POST['tarif']) ? sys::int($_POST['tarif']) : 0, 'pack' => $_POST['pack'] ?? '', 'tickrate' => isset($_POST['tickrate']) ? sys::int($_POST['tickrate']) : 0, 'slots' => isset($_POST['slots']) ? sys::int($_POST['slots']) : 0, 'time' => isset($_POST['time']) ? sys::int($_POST['time']) : 30, 'test' => (isset($_POST['time']) and $_POST['time'] == 'test') ? true : false, 'promo' => $_POST['promo'] ?? false];
...
if ($sql->num()) {
    ...
    if (isset($url['get']) and in_array($url['get'], ['price', 'promo'])) {
        $aGet = ['tarif' => sys::int($url['tarif']), 'tickrate' => sys::int($url['tickrate']), 'slots' => sys::int($url['slots']), 'time' => sys::int($url['time']), 'user' => $user['id']];
        ...
        // Определение цены
        $price = $aPrice[array_search($aGet['tickrate'], explode(':', (string) $tarif['tickrate']))];
