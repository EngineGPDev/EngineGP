...
if ($go) {
    ...
    $aData['subdomain'] = isset($_POST['subdomain']) ? strtolower((string) $_POST['subdomain']) : sys::outjs(['e' => 'Необходимо указать адрес.'], $name_mcache);
    $aData['domain'] = isset($_POST['domain']) ? strtolower((string) $_POST['domain']) : sys::outjs(['e' => 'Необходимо выбрать домен.'], $name_mcache);
    $aData['desing'] = isset($_POST['desing']) ? strtolower((string) $_POST['desing']) : sys::outjs(['e' => 'Необходимо выбрать шаблон.'], $name_mcache);
    ...
    if (strpos((string) $us['contacts'], 'ttp', 1)) {
        $vk = $us['contacts'];
        $skype = '';
    } else {
        $vk = '';
