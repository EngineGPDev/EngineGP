...
if (!$cfg['settlement_period']) {
    ...
    // Проверка периода
    if (!in_array($aData['time'], explode(':', (string) $tarif['timext'])))
...
$price = $aPrice[array_search($server['tickrate'], explode(':', (string) $tarif['tickrate']))];
...
require(SEC . 'servers/games/tarif/extend.php');
