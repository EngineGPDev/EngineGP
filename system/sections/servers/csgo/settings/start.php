...
// Вывод списка карт
if (isset($url['maps'])) {
    Game::maplist($id, $unit, $tarif['install'] . $server['uid'] . '/csgo/maps', $server['map_start'], false);
}
    ...
    switch ($url['save']) {
        case 'map':
            ...
            if ($map != $server['map_start']) {
                Game::maplist($id, $unit, $tarif['install'] . $server['uid'] . '/csgo/maps', $map, true, $nmch);
            }
        ...
        case 'fastdl':
            ...
            if ($value) {
                ...
                $ssh->setfile($temp, $tarif['install'] . $server['uid'] . '/csgo/cfg/fastdl.cfg');
                $ssh->set('chmod 0644' . ' ' . $tarif['install'] . $server['uid'] . '/csgo/cfg/fastdl.cfg');
