...
// Вывод списка карт
if (isset($url['maps'])) {
    Game::maplist($id, $unit, $tarif['install'] . $server['uid'] . '/game/csgo/maps', $server['map_start'], false);
}
    ...
    switch ($url['save']) {
        case 'map':
            ...
            if ($map != $server['map_start']) {
                Game::maplist($id, $unit, $tarif['install'] . $server['uid'] . '/game/csgo/maps', $map, true, $nmch);
            }
