...
if (!$buy and $plugin['price']) {
    ...
    $sql->query('INSERT INTO `plugins_buy` set `plugin`="' . $pid . '", `key`="' . md5(strip_tags((string) $plugin['name'])) . '", `server`="' . $id . '", `price`="' . $plugin['price'] . '", `time`="' . $start_point . '"');

    // Запись логов
    $sql->query('INSERT INTO `logs` set `user`="' . $user['id'] . '", `text`="' . sys::updtext(sys::text('logs', 'buy_plugin'),
            ['plugin' => strip_tags((string) $plugin['name']), 'money' => $plugin['price'], 'id' => $id]) . '", `date`="' . $start_point . '", `type`="buy", `money`="' . $plugin['price'] . '"');
}
