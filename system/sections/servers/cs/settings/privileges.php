...
if ($go) {
    ...
    $aData['name'] = isset($_POST['name']) ? trim((string) $_POST['name']) : sys::outjs(['e' => 'Необходимо заполнить все поля'], $name_mcache);
    $aData['flags'] = isset($_POST['flags']) ? trim((string) $_POST['flags']) : sys::outjs(['e' => 'Необходимо заполнить все поля'], $name_mcache);
    ...
    foreach (count_chars((string) $aData['flags'], 1) as $val)
        if ($val > 1)
