...
$aData['purse'] = isset($url['purse']) ? strtolower(trim((string) $url['purse'])) : sys::outjs(['e' => 'Необходимо указать кошелек'], $name_mcache);
...
if ($aData['purse'] == 'lk') {
    ...
    $sql->query('INSERT INTO `logs` set `user`="' . $user['id'] . '", `text`="' . sys::updtext(sys::text('logs', 'cashback'),
            ['purse' => $cfg['part_log'], 'money' => $aData['sum']]) . '", `date`="' . $start_point . '", `type`="cashback", `money`="' . $aData['sum'] . '"');
...
if ($cfg['part_output']) {
    ...
    if ($cfg['part_gateway'] == 'unitpay') {
        ...
        if (is_array($array) and isset($array['result']) and in_array($array['result']['status'], ['success', 'not_completed '])) {
            ...
            $sql->query('INSERT INTO `logs` set `user`="' . $user['id'] . '", `text`="' . sys::updtext(sys::text('logs', 'cashback'),
                    ['purse' => $aType[$aData['purse']], 'money' => $aData['sum']]) . '", `date`="' . $start_point . '", `type`="cashback", `money`="' . $aData['sum'] . '"');
